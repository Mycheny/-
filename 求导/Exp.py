...
class E:
    ...
    def generate(self):
        ...
        if self.type == 'float':
            ...
            if useBrack:
                E.result += '('
            # 如果是 -1*x ，直接输出 -x
            if self.left is not None and self.left == E().float(-1) and self.isOp('*'):
                E.result += '-'
            else:
                if self.left is not None:
                    self.left.generate()
    ...
    def _node_join(self, r, x, y):
        ...
        if x is None or y is None or x.type != 'float':
            return r
    ...
    def check_symbol(self, e):
        ...
        if r.type == "op":
            if r.left is not None:
                r.left = self.check_symbol(r.left)
            if r.right is not None:
                r.right = self.check_symbol(r.right)
        elif r.type == "float":
            if r.f < 0:
                r.f = -r.f
        return r
    ...
    def optm(self):  # 优化式子
        ...
        if r.left is not None and r.left == r.right:
            if r.isOp('*'):
                r.f = '^'
                r.right = E().float(2)
                r.left = self.check_symbol(r.left)
