...
def train():
    ...
    w = ad.Variable((2,), name='w')
    # w = ad.Placeholder(name='w')
    y = ad.Placeholder(name='y')
    ...
    optimizer = ad.GdOptimizer(0.1).minimize(L)

    executor = ad.Executor([optimizer])
    init_global_variable = ad.Executor([w])
    init_global_variable.run()
    ...
    for iteration in range(max_iters):
        ...
        for i in range(N):
            ...
            L_val, w_grad_val = executor.run(feed_dict={x: x_val, y: y_val})
