...
def Variable(shape, name) -> object:
    variable_node = variable_op(shape)
...
class VariableOp(Op):
    ...
    def compute(self, node, input_vals):
        """No compute function since node value is fed directly in Executor."""
        if len(input_vals) == 0:
            input_vals = np.random.random(node.shape)
            input_vals = np.array([0, 0])
        return input_vals
...
class GdOptimizer(object):
    ...
    def __init__(self, learning_rate=0.001, name='GdOptimizer'):
        self.learning_rate = learning_rate
        self.name = name

    def minimize(self, loss):
        nodes = find_topo_sort([loss])
        variables = [node for node in nodes if node.const_attr is None and hasattr(node, "shape")]
        variables_grad = gradients(loss, variables)
        for variable_grad, variable in zip(variables_grad, variables):
            variable.inputs[0] -= self.learning_rate * variable_grad
        return variables
    ...
    def run(self, feed_dict=None):
        ...
        for node in topo_order:
            if isinstance(node.op, PlaceholderOp):
                continue
            if node.const_attr is not None and hasattr(node, "shape"):
                node_to_val_map[node] = node.const_attr
            else:
                ...
                if len(node.inputs) == 0 and hasattr(node, "shape"):
                    node.const_attr = node_to_val_map[node]
