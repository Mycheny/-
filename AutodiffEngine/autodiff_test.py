...
def test_reduce_sum_mix():
    ...
    assert isinstance(y2, ad.Tensor)
...
def test_square():
    ...
    assert isinstance(y, ad.Tensor)
    assert np.array_equal(y_val, np.square(x2_val))
    assert np.array_equal(grad_x2_val, 2 * x2_val)
...
def test_pow():
    ...
    assert isinstance(y, ad.Tensor)
    assert np.array_equal(y_val, np.power(x2_val, n))
    assert np.array_equal(grad_x2_val, n * np.power(x2_val, n-1))


def test():
    x1 = ad.Variable(name="x1")
    x2 = ad.Variable(name="x2")
    const = ad.Constant(1, "const")
    y = ad.matmul(x1, x2)

    grad_x1, grad_x2 = ad.gradients(y, [x1, x2])

    executor = ad.Executor([y, grad_x1, grad_x2])
    x1_val = np.reshape([1, 2, 3], (1, 3))
    x2_val = np.reshape([2, 3, 4], (3, 1))
    y_val, grad_x1_val, grad_x2_val = executor.run(feed_dict={x1: x1_val, x2: x2_val})
    print()
