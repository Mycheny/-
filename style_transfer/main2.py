...
f_weights = K.function([], model.weights[:4])
...
for i in range(1000):
    loss_, grads_ = sess.run(outputs, feed_dict={combination_image: inp})
    inp = inp-learning_rate * grads_
    cv2.imshow("inp", deprocess_image(inp.copy())[..., ::-1])
    cv2.waitKey(1)
    if i % 50 == 0:
        img = deprocess_image(inp.copy())
